...
const audit: FormulaDefinition = {
  ...
  inputs: [
    ...
    {id:`matin`,type:`radio`,label:`6. Alcool le matin pour se remettre d\'un excès (boire matinal)`,options:[
      {value:0,label:`Jamais`},{value:1,label:`Moins d\'1 fois/mois`},{value:2,label:`1 fois/mois`},
      {value:3,label:`1 fois/semaine`},{value:4,label:`Tous les jours ou presque`},
    ]},
  ...
  calculate: (values) => {
    ...
    const sev = s >= 20 ? 'high' as const : s >= 15 ? 'moderate' as const : s >= 8 ? 'moderate' as const : 'low' as const
    return {value:s, label:`AUDIT ${s}/40`, severity: sev,
      ranges:[
        {min:0,max:7,label:`Consommation a faible risque`,severity:'low',recommendation:`Pas de probleme d\'alcool identifie. Conseils de prevention.`},
        {min:8,max:14,label:`Consommation nocive / a risque`,severity:'moderate',recommendation:`Intervention breve. Conseil de reduction de consommation. Surveillance.`},
  ...
  interpretation: `L\'AUDIT (OMS, 1989) est le gold standard pour le depistage de l\'usage d\'alcool. 10 items en 3 domaines : consommation (1-3), comportement (4-6), consequences (7-10). Score total 0-40. Seuils : 0-7 (faible risque), 8-14 (risque), 15-19 (nocif), >= 20 (dependance).`,
  clinicalCommentary: `Outil de depistage universel recommandé par l\'OMS et la HAS. Les questions 1-3 explorent la quantite/frequence. Les questions 4-6 les signes de dependance. Les questions 7-10 les consequences. Un score >= 8 chez l\'homme ou >= 7 chez la femme justifie une intervention. Attention : l\'AUDIT-C (3 premieres questions) peut servir de pre-test.`,
  references: [
